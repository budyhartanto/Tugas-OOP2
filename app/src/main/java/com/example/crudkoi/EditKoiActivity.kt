...
class EditKoiActivity : AppCompatActivity() {
    ...
    fun setupListener(){
        ...
        btn_updateKoi.setOnClickListener{
            CoroutineScope(Dispatchers.IO).launch {
                db.koiDao().updateKoi(
                    Koi(koiId, txt_jenis.text.toString(), Integer.parseInt(txt_stok.text.toString()), Integer.parseInt(txt_harga.text.toString()) )
                )
                finish()
            }
        }
    ...
    fun setupView() {
        ...
        when (intentType) {
            Constant.TYPE_CREATE -> {
                btn_updateKoi.visibility = View.GONE
            }
            Constant.TYPE_READ -> {
